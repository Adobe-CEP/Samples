	...
	mungeXMP : function(){
		...
		if (projectItem) {
			...
			if(ExternalObject.AdobeXMPScript !== undefined) { 	// safety-conscious!
				...
				var oldVal			= "";
                var oldCreatorVal = "";
				var have_a_creator	= xmp.doesPropertyExist(XMPConst.NS_DM, "creator");
				...
				if (have_a_creator === true){
					var existingCreator = xmp.getProperty(XMPConst.NS_DM, "creator");
					oldCreatorVal 		= existingCreator.value;
				}
				...
				xmp.setProperty(XMPConst.NS_DM, "creator", oldCreatorVal + " Added by PProPanel sample!");
...
//-------------
				...
				var have_a_creator			= xmp.doesPropertyExist(XMPConst.NS_DC, creatorProp);
				var firstCreator			= "PProPanel wrote the first value into creator.";
				...
				if(numCreatorValuesPresent === 0) {
					...
					oldCreatorValue	= xmp.getArrayItem(XMPConst.NS_DC, creatorProp, 1);

					xmp.setArrayItem(	XMPConst.NS_DC, 
										creatorProp, 
										1, 
										(oldCreatorValue.value + appendedText));
				}
...
//_____________
	...
	addSubClip : function() {
		...
		if (projectItem) {
			if ((projectItem.type == ProjectItemType.CLIP)	|| (projectItem.type == ProjectItemType.FILE)) {
				...
				if (newSubClip){
					newSubClip.setStartTime(12.345); // New in 11.0
				}
			} else {
			...
			if (firstProjectItem.canProxy()){
				...
				if (shouldAttachProxy) {
					...
					if (proxyPath.exists){
						...
						// var proxyPath = firstProjectItem.getProxyPath(); // New in 11.0
					} else {
				...
				if (firstClip){
					...
					if (clipComponents){
						...
						if (clipComponents.numItems > 2){
							// Assume Gaussian Blur is the first effect applied to the clip.
							// 0 = 
							// 1 = Opacity
							...
							alert("Please apply the effect \'Gaussian Blur\' to the first clip in the first video track of the active sequence.");
						}
					}
				}
			}
	...
	getJustFileName : function (fullPath){
			...
			if (fileOrFilesToImport) {
				...
				if (targetBin){
					...
					if (importThese){
						for (var i = 0; i < fileOrFilesToImport.length; i++) {
							importThese[i]			= fileOrFilesToImport[i].fsName;
							var justFileName		= getJustFileName(importThese[i]);
